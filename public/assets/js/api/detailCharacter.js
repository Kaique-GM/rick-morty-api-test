...
export async function initCharacter() {
    ...
    const editCharacterBtn = document.getElementById("editButton");
    ...
    if (!manageUserButtons || !saveCharacterBtn || !deleteCharacterBtn || !editCharacterBtn) return;
    ...
    deleteCharacterBtn.addEventListener("click", async () => {
        ...
        if (response.deleted) {
            ...
            mensageText.classList.remove("editMensage");
    ...
    editCharacterBtn.addEventListener("click", async () => {
        if (!userId) {
            window.location.href = "/login";
            return;
        }

        await editCharacter(character, userId);
    });
}
...
async function editCharacter(character, userId) {
    let api_id = character.id
    let user_id = userId;

    const detailSection = document.getElementById("detailSection");

    const existingForm = document.querySelector(".formSection");
    if (existingForm) {
        existingForm.remove();
        mensageText.classList.remove("editMensage");
        mensageText.classList.remove("sucessMensage");
        mensageText.classList.add("failMensage");
        mensageText.textContent = "Formulário de editação fechado!";
        return;
    }
    ...
    formSection.innerHTML = `
            <div class="formBoxDetails">
                ...
                <form id="editForm">
                    <label>Nome</label> 
                    <input id="characterName" type="text" placeholder="Nome" value="${character.name || ''}">
                    <label>Espécie</label> 
                    <input id="characterSpecies" type="text" placeholder="Espécie" value="${character.species || ''}">
                    <label>Imagem (Apenas URL)</label> 
                    <input id="characterImg" type="text" placeholder="Imagem (Apenas URL)" value="${character.image || ''}">
                    <label>Url do personagem</label> 
                    <input id="characterURL" type="text" placeholder="Url do personagem" value="${character.url || ''}">
    ...
    detailSection.appendChild(formSection);
    ...
    mensageText.classList.remove("sucessMensage");
    mensageText.classList.remove("failMensage");
    mensageText.classList.add("editMensage");
    mensageText.textContent = "Formulário de editação aberto!";
    ...
    form.addEventListener("submit", async (e) => {
        ...
        const species = document.getElementById("characterSpecies").value;
        const image = document.getElementById("characterImg").value;
        const url = document.getElementById("characterURL").value;

        if (!name || !species || !image || !url) {
            mensageText.textContent = "Preencha todos os campos!";
            mensageText.classList.remove("editMensage");
            mensageText.classList.remove("sucessMensage");
            mensageText.classList.add("failMensage");
            window.scrollTo({
        ...
        const response = await fetch("/editCharacter", {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ api_id, user_id, name, species, image, url })
        });

        const data = await response.json();

        if (data.edited) {
            mensageText.textContent = "Personagem editado com sucesso!";
            mensageText.classList.remove("editMensage");
            mensageText.classList.remove("failMensage");
            mensageText.classList.add("sucessMensage");
            window.scrollTo({
            ...
            const response = await checkCharacter(api_id, user_id);
            character = response.character;
